...
if (opt$pop == "cp"){
	d<-read.cross(mapfile=paste(opt$mark,"map","."),genfile=paste(opt$mark,"loc","."),phefile=paste(opt$mark,"trt","."),format="csvsr",crosstype=opt$pop,na.string="NaN")
}else{
	d<-read.cross(file=paste(opt$mark,"csv","."),phefile=paste(opt$mark,"trt","."),format="csvsr",crosstype=opt$pop,na.string="NaN")
}
