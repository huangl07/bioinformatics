...
open SH,">$dsh/step01.vcf-filter.sh";
    print SH "vcftools --remove-filtered-all --remove-indels --minDP $dep  --max-missing $mis --vcf $vcf --recode  --out $out/pop --maf $maf  && perl $Bin/bin/vcf2hamp-ms.pl  -i $out/pop.recode.vcf -o $out/pop.msmc.txt \n";
close SH;
