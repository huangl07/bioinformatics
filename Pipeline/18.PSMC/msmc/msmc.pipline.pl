...
if ($step ==2) {
	...
	print Log "vcf-to-ms \n",my $time=time();
	print Log "########################################\n";
	my $ms=ABSOLUTE_DIR("$out/step01.vcf-filter/pop.msmc.txt");
	my $job="perl $Bin/bin/step02.vcf2ms.pl  -i $ms  -g $group -m 1 -out $out/step02.vcf2ms -dsh $out/work_sh ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
	print Log "vcf-to-ms Done and elapsed time : ",time()-$time,"s\n";
	print Log "########################################\n";
    $step++ if ($step ne $stop);
}
if ($step ==3) {
	...
	print Log "msmc \n",my $time=time();
	print Log "########################################\n";
	my $msmc=ABSOLUTE_DIR("$out/step02.vcf2ms/");
	my $job="perl $Bin/bin/step03.msmc.pl -i $msmc -g $group  -out $out/step03.msmc -dsh $out/work_sh ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
	print Log "msmc- Done and elapsed time : ",time()-$time,"s\n";
	print Log "########################################\n";
	$step++ if ($step ne $stop);
}
#if ($step ==4) {
