...
my %pos;
my %sum;
while (<In>) {#按照window和step划分windows,第n个windows位于n*step~n*step+win之间
	chomp;
	next if ($_ eq ""||/^$/ || /^#/);
	my ($id,$chr,$position)=split;
	$sum{$chr}++;
...
my %wintype;
foreach my $chr (sort keys %pos) {
	if (scalar keys %{$pos{$chr}} == 1) { #一个scaffold上只有一个windows的情况,无法补数据，缺失保留
