...
library(ape)
library(ade4)
d1<-read.table(opt$dif,head=FALSE,row.names=1);
...
for (i in 2:length(d2.names)){
	...
	mantel<-mantel.test(d1, matrix2, graph = FALSE)
	write.table(file=paste(d2.names[i],".mental.xls",sep=""),data.frame(name=d2.names[i],z.stat=mantel$z.stat,p=mantel$p),row.names=FALSE);
	pdf(paste(d2.names[i],".pdf",sep=""));
	plot(unlist(d1),as.vector(unlist(matrix2)),main="Mantel test",ylab=d2.names[i],xlab="genetic distance");
	d<-data.frame(x=unlist(d1),y=as.vector(unlist(matrix2)))
