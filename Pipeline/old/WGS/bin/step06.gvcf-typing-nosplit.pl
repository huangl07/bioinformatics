...
open SH,">$dShell/step06.vcf-typing.sh";
open In,$gvcflist;
...
print SH "java -Djava.io.tmpdir=$dOut/tmp/ -Xmx35G  -jar /mnt/ilustre/users/dna/.env/bin/GenomeAnalysisTK.jar -T GenotypeGVCFs $Gvcf -o $dOut/pop.noid.vcf -R $ref -log $dOut/pop.variant.log && ";
print SH "bcftools annotate --set-id +\'%CHROM\\_%POS' $dOut/pop.noid.vcf -o $dOut/pop.variant.vcf\n";
close SH;
close Out;
my $job="perl /mnt/ilustre/users/dna/.env/bin/qsub-sge.pl  --Resource mem=35G --CPU 1  --maxjob $proc $dShell/step06.vcf-typing.sh";
`$job`;
#######################################################################################
