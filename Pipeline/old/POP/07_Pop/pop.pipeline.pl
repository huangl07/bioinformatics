...
if ($step ==1) {
	...
	print Log "########################################\n";
	$vcf=ABSOLUTE_DIR("$out/step00.vcf-filter/vcf.list");
	my $job="perl $Bin/bin/step01.vcf-convert.pl -vcf $vcf -out $out/step01.vcf-convert -dsh $out/work_sh -chr $chr ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if ($step ==2) {
	...
	print Log "########################################\n";
	$vcf=ABSOLUTE_DIR("$out/step01.vcf-convert/pop.list");
	my $sam=ABSOLUTE_DIR("$out/step01.vcf-convert/sample.list");
	my $job="perl $Bin/bin/step02.structure-generic.pl -pop $vcf -out $out/step02.structure-generic -dsh $out/work_sh -sam $sam";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if ($step ==3) {
	...
	my $job="perl $Bin/bin/step03.PCA-generic.pl -vcf $vcf -out $out/step03.pca-generic -dsh $out/work_sh -chr $chr ";
	$job.="-gro $gro\n" if ($gro);
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
