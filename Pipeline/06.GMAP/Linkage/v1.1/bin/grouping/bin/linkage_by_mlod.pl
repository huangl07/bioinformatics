...
# GetOptions
# ------------------------------------------------------------------
my ($fIn,$fKey,$genotype,$dOut,$start,$end,$stepSize,$minGroup,$maxGroup,$nChro,$redo,$log,$mode);
GetOptions(
				"help|?" =>\&USAGE,
				"i:s"=>\$fIn,
				"g:s"=>\$genotype,
...
if (-f "$dOut/$fKey.gTree.hash" && not defined $redo){
...
if (-f "$dOut/$fKey.gTree.delFragment.hash" && $succeed_flag == 1 && not defined $redo) {
...
if (-f "$dOut/$fKey.gTree.delFragment.mergeNode.hash" && $succeed_flag == 1 && not defined $redo){
