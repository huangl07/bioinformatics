...
# GetOptions
# ------------------------------------------------------------------
my ($fIn1,$fIn2,$fIn3,$outdir,$fOut,$fOut1,$Reverse,$switch,$genotype);
GetOptions(
				"help|?" =>\&USAGE,
				"i1:s"=>\$fIn1,
				"i2:s"=>\$fIn2,
...
if ($genotype) {
	...
	while (<GE>) {
		chomp;
		next if (/^$/ or /^MarkerID/ or /^;/ or /^name/ or /^popt/ or /^nloc/ or /^nind/);
		my ($marker,$type,@useless)=split;
		if ($type=~/\<(.*)\>/) {
			$genotype{$marker}=$1;
		}else{
	...
	warn "The genotype file is not given,please check!"	;
	die;
}
#print Dumper %genotype;die;
...
WHILE2:	while (<IN3>) {
	...
	while (<IN2>) {
		...
		$all{$file2_group}{"7"}{$1}=$2 if ( /^(\w+?\d+)\s+(\S+)/);
	} 
	close(IN2);
...
#	print @tem,"\n";die;
	...
	my $PaperWidth=$LeftMargin+$RightMargin+($MarkerTxtSpace*6+$MarkerLineSpace*6+$ChroWidth*3+$space_betw_chro*2)+$CmTxtSpace*3+$Marker_LineSpace*6;
	...
	my $y=$TitleHeight+$titleHeight+$Title_title_space+$title_chro_space+$TopMargin+$ChroWidth/2;
	...
	print SVG &svg_txt($PaperWidth/2,$TopMargin+$TitleHeight,$TextSize,$TextColor,"LG$lg_num");
	my $x1=$LeftMargin+$MarkerLineSpace+$MarkerTxtSpace*2+$Marker_LineSpace;
	my $x2=$LeftMargin+$MarkerLineSpace*3+$MarkerTxtSpace*3+$space_betw_chro+$ChroWidth+$CmTxtSpace+$Marker_LineSpace*3;
	my $x3=$LeftMargin+$MarkerLineSpace*5+$MarkerTxtSpace*4+$space_betw_chro*2+$ChroWidth*2+$CmTxtSpace*2+$Marker_LineSpace*5;
	...
	foreach my $whichK (sort {$a <=> $b} keys %{$all{$groups}}) {
		...
		foreach my $marker(sort {$all{$groups}{$whichK}{$a} <=> $all{$groups}{$whichK}{$b}} keys  %{$all{$groups}{$whichK}} ){
			...
			if ($whichK eq "5") {
				...
				my $MY=$length*$k;
				print SVG &svg_txt($LeftMargin,$MY+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$TextSize,"$TextColor","$genotype{$marker}");
				print SVG &svg_txt($LeftMargin+$MarkerTxtSpace,$MY+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$TextSize,"$TextColor","$marker");
				...
				if ($pos{$groups}{"7k"} eq "reverse") {
					...
					print SVG &svg_txt($x2-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace,$bottomY-$MY,$TextSize,"$TextColor","$genotype{$marker}");
					...
					print SVG &svg_txt($x2-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace,$MY+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$TextSize,"$TextColor","$genotype{$marker}");
			...
			$k++;
				...
				if ($pos{$groups}{"9k"} eq "reverse") {
					...
					print SVG &svg_txt($x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace,$bottomY-$MY,$TextSize,"$TextColor","$genotype{$marker}");
					...
					print SVG &svg_txt($x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace,$MY+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$TextSize,"$TextColor","$genotype{$marker}");
	...
	my $A_num=keys %{$all{$groups}{"7"}};
	my $F_num=keys %{$all{$groups}{"5"}};
	my $M_num=keys %{$all{$groups}{"9"}};
	print SVG &svg_txt($x3+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$MarkerTxtSpace,$y,$TextSize,"red","SexAver : $A_num");
	print SVG &svg_txt($x3+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$MarkerTxtSpace,$y+30,$TextSize,"red","Female : $F_num,${$gong{$groups}}[0]");
	print SVG &svg_txt($x3+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$MarkerTxtSpace,$y+60,$TextSize,"red","Male : $M_num,${$gong{$groups}}[1]");

	foreach my $marker1 (@marker1) {
		if (defined $pos{$groups}{"7Ynew"}{$marker1}) {
			print SVG &svg_line($x1+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"5"}{$marker1}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$x2-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"7Ynew"}{$marker1},$LineColor,$LineSize);
		}elsif (defined $all{$groups}{"7"}{$marker1}) {
			print SVG &svg_line($x1+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"5"}{$marker1}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$x2-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"7ynoreverse"}{$marker1}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$LineColor,$LineSize);
		}
	}
	if ($pos{$groups}{"7k"} eq "reverse") {
		foreach my $marker2 (@marker2) {
			if (defined $pos{$groups}{"9Ynew"}{$marker2}) {
				print SVG &svg_line($x2+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"7Ynew"}{$marker2},$x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"9Ynew"}{$marker2},$LineColor,$LineSize);
			}elsif (defined $all{$groups}{"9"}{$marker2}) {
				print SVG &svg_line($x2+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"7Ynew"}{$marker2},$x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"9ynoreverse"}{$marker2}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$LineColor,$LineSize);
			}
		}
	}else{
		foreach my $marker2 (@marker2) {
			if (defined $pos{$groups}{"9Ynew"}{$marker2}) {
				print SVG &svg_line($x2+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"7ynoreverse"}{$marker2}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"9Ynew"}{$marker2},$LineColor,$LineSize);
			}elsif (defined $all{$groups}{"9"}{$marker2}) {
				 print SVG &svg_line($x2+$ChroWidth+$MarkerLineSpace+$Marker_LineSpace+$CmTxtSpace,$pos{$groups}{"7ynoreverse"}{$marker2}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$x3-$MarkerLineSpace-$MarkerTxtSpace*2-$Marker_LineSpace-4,$pos{$groups}{"9ynoreverse"}{$marker2}+$TopMargin+$TitleHeight+$titleHeight+$ChroWidth/2+$Title_title_space+$title_chro_space,$LineColor,$LineSize);
			}
		}
	}
	print SVG &svg_end ();
...
Usage:map.pl <可用于画三个图谱间的共线性关系，从左往右依次是雌中雄,运行时不输入文件，会在当前路径下找形如*.female.map、*.male.map、*.sexAver.map的文件，并绘图>
  ...
  -g <file>	   genotype file with crross_type of all markers ,forced 
