...
my ($fIn,$fOut,$threshold,$windows_size,$min_mis,$map,$fKey);
GetOptions(
				"help|?" =>\&USAGE,
				"i:s"=>\$fIn,
				"m:s"=>\$map,
...
while (<LOC>) {
	...
	my ($marker,undef,@genotype) = split /\s+/ , $_ ;
	next if (@genotype < 6) ;
...
while (<Map>) {
	chomp;
	next if (/^$/ || /group/);
	my ($ID,$pos,undef)=split(/\s+/,$_);
	$Posi{$ID}=$pos;
}
close Map;
my @marker=sort{$Posi{$a}<=>$Posi{$b}} keys %Posi;
#
# score weights 
#
my %weight = (
...
$miss_num{before} = 0;
$miss_num{after}  = 0;
open (Nloc,">","$fOut/$fKey.genotype") or die $!;
print Nloc $head,"\n";
open (Sta,">","$fOut/$fKey.genotype.detail") or die $!;

for (my $i=0; $i<@marker ; $i++) {
	print Nloc $marker[$i] ,"\t" ,"aaxbb\t";
	print Sta $marker[$i],"\t" ;
	foreach my $pos (sort {$a <=> $b} @pos) {
	   my $out=$genotype_new{$pos}{$marker[$i]};
	   $out =~ s/a/aa/g;
	   $out =~ s/b/bb/g;
   	   $out =~ s/h/ab/g;
	   $out =~ s/-/--/g;
...
Usage: 根据具有map顺序的loc文件进行基因型纠错
  ...
  -miss <float>  minimim miss ratio, [0.85]
  -h          Help
