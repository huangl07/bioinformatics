...
# GetOptions
# ------------------------------------------------------------------
my ($fIn,$fKey,$error_ratio,$dOut,$mode,$cycle_t,$log);
GetOptions(
				"help|?" =>\&USAGE,
				"i:s"=>\$fIn,
				"k:s"=>\$fKey,
...
if ($mode == 0) {
	if ($file_type eq 'loc_lp') { ## 输入文件为带连锁相信息的loc文件
		...
		## construct map using sgsMap
		{
			...
			print "$Bin/sgsMap -loc $fIn -pwd $dOut/pwd/$fKey.pwd -k $dOut/map/$fKey\n";
			print $log "$Bin/sgsMap -loc $fIn -pwd $dOut/pwd/$fKey.pwd -k $dOut/map/$fKey\n";
			`$Bin/sgsMap -loc $fIn -pwd $dOut/pwd/$fKey.pwd -k $dOut/map/$fKey`;
		...
		{
			print "perl $Bin/smooth.pl -m $dOut/map/$fKey.sexAver.map -l $fIn -k $fKey -d $dOut/smooth -cycle_threshold 3 -sus $error_ratio\n";
			print $log "perl $Bin/smooth.pl -m $dOut/map/$fKey.sexAver.map -l $fIn -k $fKey -d $dOut/smooth -cycle_threshold 3 -sus $error_ratio\n";
			`perl $Bin/smooth.pl -m $dOut/map/$fKey.sexAver.map -l $fIn -k $fKey -d $dOut/smooth -cycle_threshold 3 -sus $error_ratio`;
		...
		## Order Marker in a linkage group using sgsMap
		{
			...
			print "$Bin/sgsMap -loc $dOut/phase/$fKey.loc -pwd $dOut/pwd/$fKey.modified.pwd -k $dOut/map/$fKey\n";
			print $log "$Bin/sgsMap -loc $dOut/phase/$fKey.loc -pwd $dOut/pwd/$fKey.modified.pwd -k $dOut/map/$fKey\n";
			`$Bin/sgsMap -loc $dOut/phase/$fKey.loc -pwd $dOut/pwd/$fKey.modified.pwd -k $dOut/map/$fKey`;
	...
	while (++$cycle < $cycle_t) {
		...
		if ($file_type eq 'loc_lp') {
			print "perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.loc -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0\n";
			print $log "perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.loc -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0\n";
			`perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.loc -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0`;
		}else{
			print "perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.genotype -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0\n";
			print $log "perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.genotype -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0\n";
			`perl $0 -i $dOut/cycle_$last_cycle/smooth/$fKey.correct.genotype -k $fKey -d $current_cycle_dir -e $error_ratio -mode 0`;
		}	
	}
	...
	## map
	`ln -s $dOut/cycle_$cycle/map/*.map $dOut/result`;
	## loc
	`ln -s $dOut/cycle_$cycle/smooth/*.loc $dOut/result/$fKey.female.loc`;
	`ln -s $dOut/cycle_$cycle/smooth/*.loc $dOut/result/$fKey.male.loc`;
	`ln -s $dOut/cycle_$cycle/smooth/*.loc $dOut/result/$fKey.sexAver.loc`;
