...
if(!defined($opts{i}) || !defined($opts{o}) || defined($opts{h}))
{
	print <<"	Usage End.";
	Description:
		This script is write to change block format for genetic map process input.
		Version: $ver
...
while (<IN>){
	chomp ;
	next if (m/^\s*$/);
	my ($marker_id, $chr, $pos, $type, $P, $M, @types) = split ;
	if ($marker_id =~ /ID$/i){
		print OUT "MarkerID\t$type\t", join("\t", @types), "\n" ;
	}
	else{
		print OUT "$marker_id\t$type\t", join("\t", @types), "\n" ;
	}
}
close(IN);
close(OUT);
