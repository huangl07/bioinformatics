...
if(!defined($opts{i}) || !defined($opts{o}) || defined($opts{h}))
{
	print <<"	Usage End.";
	Description:
		This script is write to get posi file from marker file.
		Version: $ver

	Usage:

		-i           marker or block info file           <infile>     must be given
		-o           result posi file                    <outfile>    must be given
		-h           Help document
...
## reading marker info and get posi
open (IN, $infile) || die "Can't open $infile, $!\n" ;
open (OUT, ">$outfile") || die "Can't creat $outfile, $!\n" ;
while (<IN>){
	chomp ;
	next if (m/^\#/ || m/^\s*$/);
	my ($marker_id, $chr, $pos) = split ;
	next if ($marker_id =~ /ID/i);
	if ($pos =~ /\-/){
		my ($start, $end) = split /\-/, $pos ;
		print OUT "$marker_id\t$chr\t$start\t$end\n" ;
	}
	else{
		print OUT "$marker_id\t$chr\t$pos\t", ($pos+1), "\n" ;
	}
}
close(IN);
close(OUT);
