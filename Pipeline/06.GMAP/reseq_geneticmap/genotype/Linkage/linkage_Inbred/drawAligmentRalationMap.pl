...
&USAGE unless ($fMap and $fAlign  and $fKey);
...
# ------------------------------------------------------------------
#my %chroLength;
open (IN,"<",$fAlign) or die $!;
while (<IN>) {
	chomp;
	next if (/^\#/ || /^$/) ;
	my ($marker,$scaffold,$starts,undef)=split /\s+/,$_;

	next if (not exists $Marker{$marker} || $marker eq "") ;
	$Alignment{$scaffold}{$marker}=$starts;
	$chroLength{$scaffold}=$starts if (!exists $chroLength{$scaffold} || $chroLength{$scaffold} < $starts);
	$Marker{$marker}{"C"}{$scaffold}=$starts;
...
if ($PNG) {
	...
	chdir $dOut;
	`$ConvertSVGtoPNG $fKey.svg -t png`;
	chdir $pwd;
}
