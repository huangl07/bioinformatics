...
my $EndMarkerCurr = ($OnlyFirst ==0) ? $k-1 : $OnlyFirst;
...
for (my $i=0; $i<$EndMarkerCurr; $i++) {
	for (my $j=$i+1; $j<$k; $j++) {
		...
		## skip lmxll & nnxnp 
		if (($genotype1 eq "lmxll" && $genotype2 eq "nnxnp")|| ($genotype1 eq "nnxnp" && $genotype2 eq "lmxll")) {
			print OUT"$marker1\t$marker2\t-1\t-1\t",-1,"\n";
			next;
		}
		...
		if ($debug) {
			next unless (($marker1 eq 'Marker100008' and $marker2 eq 'Marker100010') or ($marker1 eq 'Marker100010' and $marker2 eq 'Marker100008')) ;
			print ">loci_1: $marker1\n";
			foreach my $geno (keys %R) {
				print "$geno => $R{$geno}\n";
			}

			print ">loci_2: $marker2\n";
			foreach my $geno (keys %C) {
				print "$geno => $C{$geno}\n";
			}

			print "geno combination :\n";
			foreach my $com (keys %pairs) {

				print "$com => $pairs{$com}\n";
			}
			
			print "sum of matix : $T\n";
		}
		############################
		...
		print "mLOD = $mlod\n" if ($debug) ;
		print OUT"$marker1\t$marker2\t$mlod\t$G\t",$all{$i}{"genotype_num"}*$all{$j}{"genotype_num"},"\n";
	}
}
