...
void BCpxFy_dist_cal::count_class(){
...
void BCpxFy_dist_cal::upper_bound(){
...
void BCpxFy_dist_cal::lower_bound(){
...
void BCpxFy_dist_cal::expected_CDEFG(double delta,
                                  double& C,
                                  double& D,
                                  double& E,
                                  double& F,
                                  double& G) const {
    assert((BC_generation_index_ >= 1) and (BC_generation_index_ <= 99));
    assert((F_generation_index_ >= 0) and (F_generation_index_ <= 99));
    // assert((p_ == 'A') or (p_ == 'B') or (p_ == 'a') or (p_ == 'b'));
	//cout << "bc generation: " << BC_generation_index_ << endl;
	//cout << "self generation: " << F_generation_index_ << endl;
    ...
    double FF = 1.0;
    double GG = 0.0;
    // backcross x generations 
    for (int i = 0; i < BC_generation_index_; i++) {
        double CC_prime = CC + 0.5 * EE + 0.5 * (1 - delta) * FF;
        double DD_prime = 0;
        double EE_prime = 0.5 * EE + 0.5 * delta * FF;
        double FF_prime = 0.5 * (1 - delta) * FF;
        double GG_prime = 0;
	...
	cout << "FF: " << FF << endl;
	cout << "GG: " << GG << endl;
	double tt = 2 * CC + 2 * DD + 4 * EE + FF + GG;
	cout << "total: " << tt << endl;
    */
	// selfing y generations 
    for (int ii = 0; ii < F_generation_index_; ii++) {
...
double BCpxFy_dist_cal::squared_error(double delta) const {
...
double BCpxFy_dist_cal::find_opt_delta() const {
...
linkage_group_BCpxFy::linkage_group_BCpxFy(int _number_of_bins, 
...
linkage_group_BCpxFy::~linkage_group_BCpxFy(){
    return;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

void linkage_group_BCpxFy::calculate_pair_wise_distance(){
    pair_wise_distances.resize(number_of_bins);
    for (int ii = 0 ; ii < number_of_bins; ii++)
    {
        pair_wise_distances[ii].resize(number_of_bins);
    ...
    for (int ii = 0 ; ii < number_of_bins; ii++)
    {
        ...
        {
            BCpxFy_dist_cal bcpxfy_dist(BC_generation_index_, F_generation_index_, raw_data[ii], raw_data[jj]);
...
void linkage_group_BCpxFy::estimate_missing_data() {
    // need to do something here
    return;
};
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

void linkage_group_BCpxFy::dump() const {
    dump_common();
    cout << "backcross generation_index:" << BC_generation_index_ << endl;
    cout << "selfing generation_index:" << F_generation_index_ << endl;
...
void linkage_group_BCpxFy::order_markers() {
