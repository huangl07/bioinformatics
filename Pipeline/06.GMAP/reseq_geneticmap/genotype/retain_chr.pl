...
if(!defined($opts{i}) || !defined($opts{o}) || defined($opts{h}))
{
	print <<"	Usage End.";
	Description:
		This script is write to filter scaffold and retain chr genotype result.
		Version: $ver
	...
	Usage:
		...
		-m           key word, default 'Chr'       optional
		-h           Help document
...
my $infile  = $opts{i} ;
my $outfile = $opts{o} ;
my $word = defined $opts{m} ? $opts{m} : 'Chr' ;

## reading infile and retain lines contain $word.
