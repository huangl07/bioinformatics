...
fstboost<-function (popdata, fst.method = "EBFST", bsrep = 100, log.bs = F) 
{
    ...
    gp_locind_resample <- function(popdata, crep) {
        ...
        for (cpop in 1:numpop) {
            ...
            cgp <- cbind(indnames[[cbspop]][select_ind], ",", 
                cgp[select_ind, ])
        ...
        if (log.bs) {
            file.rename("finepopbs.tmp", paste0("gtdata_bs", 
                crep, ".txt"))
        }
        else {
            file.remove("finepopbs.tmp")
        }
    ...
    return(list(org.fst = fst0, bs.fst.list = bs.fstlist, bs.pop.list = bs.poplist))
}
...
fstbs <- fstboost(popdata)
DJ=DJ(popdata)
write.table(file=paste(opt$output,"FstBoost.matrix",sep="."),fstbs,sep="\t");
