...
open SH,">$dsh/step01.vcf-filter.sh";
    print SH "vcftools --remove-filtered-all --remove-indels --minDP $dep  --max-missing $mis --vcf $vcf --recode  --out $out/pop --maf $maf  && perl $Bin/bin/rebuild.pop.pl -i $group -o $out/group.list && python /mnt/ilustre/users/nanshan.yang/newmdt/Pipline/03.history/treemix/bin/bin/vcf2treemix.py -vcf $out/pop.recode.vcf -pop $out/group.list -out $out && cd $out && gzip $out/pop.tmix \n";
close SH;
