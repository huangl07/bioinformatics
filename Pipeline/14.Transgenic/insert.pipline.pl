...
if ($step ==3) {
	...
	my $job="perl $Bin/bin/step03.bam-insert.pl -bamlist $bamlist -fqlist $fqlist -out $out/step03.bam-insert -dsh $out/work_sh ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if ($step ==4) {
	...
	print Log "########################################\n";
    my $fq=ABSOLUTE_DIR("$out/step03.bam-insert/insert.fq.list");
	my $job="perl $Bin/bin/step04.soap-denovo.pl -fqlist $fq -out $out/step04.soap-denovo/ -dsh $out/work_sh ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if($step ==5){
    ...
    print Log "blast \n",my $time=time();
    print Log "########################################\n";
    my $list=ABSOLUTE_DIR("$out/step04.soap-denovo/soap.list");
    my $db="$out/step01.index/dbname";
    my $job="perl $Bin/bin/step05.blast.pl -list $list -db $db -out $out/step05.blast -dsh $out/work_sh";
    print Log "$job\n";
    `$job`;
    print Log "$job\tdone!\n";
    print Log "########################################\n";
