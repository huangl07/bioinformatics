...
open SH,">$dShell/step01.bwa-mapping.sh";
open In,$fqlist;
...
close Out;
my $job="perl /mnt/ilustre/users/dna/.env/bin/qsub-sge.pl  --Resource mem=5G --CPU 8 --maxjob $proc $dShell/step01.bwa-mapping.sh";
`$job`;
#######################################################################################
