...
open SH,">$dShell/step09.sv-calling.sh";
open In,$bamlist;
...
while (<In>) {
	...
	print SH "perl $Bin/bin/bam2cfg.pl -v 5 $bam > $dOut/$sampleID.cfg && ";
...
close SH;
close List;
my $job="perl /mnt/ilustre/users/dna/.env/bin/qsub-sge.pl --Resource mem=20G --CPU 1  --maxjob $proc $dShell/step09.sv-calling.sh";
`$job`;
#######################################################################################
