...
while (<In>){
	...
	my ($chr,undef,$type,undef,$size,$start,$end)=split(/\s+/,$_);
	$Chrinfo{$chr."_".$start}=join("\t",$end,$type,$size);
	push @{$CHR{$chr}},$start;
}	
close In;
open Ref,"$ref";
...
#	my($size,$start);  # add ID;
	if(exists $CHR{$chr}){
		foreach my $start (sort @{$CHR{$chr}}){
			my($end,$type,$size)=split /\t/,$Chrinfo{$chr."_".$start};
...
##$type+length($size)-1+20-($START_pos)
			...
			my $ID=join("_",$chr,$type,$size,$start,$end);
