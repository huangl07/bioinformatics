...
print OUT "#Chr\tSSR type\tSize\tStart\tEnd\t";
print OUT "FORWARD PRIMER1 (5'-3')\tTm(°C)\tGC(%)\tLength(bp)\tREVERSE PRIMER1 (5'-3')\tTm(°C)\tGC(%)\tLength(bp)\tPRODUCT size(bp)\n";

undef $/;
$/ = "=\n";
while (<SRC>)
  {
  my @misa= (/PRIMER_SEQUENCE_ID=(\S+)_(\S+)_(\S+)_(\S+)_(\S+)/);
