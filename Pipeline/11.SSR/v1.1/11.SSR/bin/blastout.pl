...
open In1,$fIn1;
open In2,$fIn2;
open Out1,">$fOut.misa-1.out";
open Out2,">$fOut.misa-2.out";
my %hash;
while(<In1>){
	my @line=split(/\t/);
...
while(<In2>){
	next if (/^$/ or "");
	my @line=split(/\t/,$_,2);
	my ($sca,$pos)=split(/\:/,$line[0]);
	if (exists $hash{$sca}){
		my @long=split(/\t/,$hash{$sca});
		if ($pos>= $long[6] and $pos<=$long[7]){
			my $dis=int($pos-$long[6]);
			if ($long[8] > $long[9]){
				my $pos2=int($long[9]+$dis);
				print Out1"$key\t$long[1]:$pos2\t$line[-1]";
			}
			else{
