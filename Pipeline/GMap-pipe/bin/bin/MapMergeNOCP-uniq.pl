...
foreach my $map (@map) {
	...
	foreach my $nr (@order) {
		my ($id,$pos)=split(/\,/,$Marker{$nr});
		my ($nextid,$nextpos)=split(/\,/,$Marker{$nr+1});
		$pos=$newdis/$max*$pos;
		$Marker{$nr}=join(",",$id,$pos);
		#print Out $id,"\t",$pos,"\n";
		if($nextpos eq $pos){
...
foreach my $marker (@marker) {
	...
	while (<In>) {
		...
		if (/MarkerID/) {
			...
			if (!exists $Marker{$nr}{$id}) {
				next;
			}
			my $number=(split(/\D+/,(split(/\_/,(split(/\,/,$Marker{$nr}))[0]))[1]))[-1];
			my $pos=(split(/\,/,$Marker{$nr}))[1];
			my $nextpos=(split(/\,/,$Marker{$nr+1}))[1];
			if ($nextpos eq $pos){
				next;
			}else{
				...
				push @{$stat{$number}{info}},join(",",$id,$Marker{$nr},$info);
			}
		}
	}
	close In;
