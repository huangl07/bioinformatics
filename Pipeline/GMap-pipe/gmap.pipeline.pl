...
my $BEGIN_TIME=time();
use Getopt::Long;
my ($vcf,$popt,$out,$bin,$pid,$mid,$nchr,$ref,$list,$type,$step);
use Data::Dumper;
...
if ($step == 7) {
	...
	print Log "########################################\n";
	my $gen=ABSOLUTE_DIR("$out/06.map-cycle1/marker.list");
...
if ($step == 8) {
	...
	print Log "########################################\n";
	my $gen=ABSOLUTE_DIR("$out/07.map-cycle2/marker.list");
...
if ($step == 10){
	print "########################################\n";
	print Log "merge result\n",my $time=time();
	print Log "########################################\n";
	my $dmap=ABSOLUTE_DIR("$out/10.mapEvalue");
	my $marker=ABSOLUTE_DIR("$out/01.vcf-convert");
	my $job="perl $Bin/bin/step08.merge-result.pl -dmap $dmap -list $list -vcf $vcf -marker $marker -popt $popt -type $type -out $out/11.report -dsh $dsh ";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
