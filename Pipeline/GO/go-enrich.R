...
names(data)<-c("id","description","source","k","M","n","N");
data<-na.omit(data)
...
outd<-data.frame(id=data$id,des=data$description,source=data$source,eff=data$k,total=data$M,pvalue=1-pvalue,qvalue=1-qvalue)
order<-c(1:length(outd$des))
...
p1<-ggplot(outd,aes(x=des,y=eff,colour=source))+geom_point(aes(size=eff))+facet_wrap( ~ source)+theme_bw()+theme(legend.position='none')+theme(plot.title=element_text(hjust=0.5))+coord_flip()
ggsave(filename=paste(opt$output,"buble.pdf",sep="."),p,height=9,width=16,device="pdf")
ggsave(filename=paste(opt$output,"buble.png",sep="."),p,height=9,width=16,device="png")

outd$des<-factor(outd$des,levels<-outd$des[order],ordered=T)
if(is.null(opt$eggnog)){
	p<-ggplot(outd,aes(x=factor(des),y=pvalue,fill=source))
	p<-p+theme_bw()+geom_bar(stat="identity",width=0.5)
...
ggsave(filename=paste(opt$output,".bar.pdf",sep="."),p,height=9,width=16,device="pdf")
ggsave(filename=paste(opt$output,".bar.png",sep="."),p,height=9,width=16,device="png")
