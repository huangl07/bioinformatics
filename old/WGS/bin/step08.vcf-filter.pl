...
open SH,">$dShell/step08.vcf-filter1.sh";
...
close List;
my $job="perl /mnt/ilustre/users/dna/.env//bin/qsub-sge.pl --Resource mem=20G --CPU 1  --maxjob $proc $dShell/step08.vcf-filter1.sh";
print $job;
`$job`;
open SH,">$dShell/step08.vcf-filter2.sh";
print SH "java -Xmx10G -jar /mnt/ilustre/users/dna/.env/bin/GenomeAnalysisTK.jar -nt 8 -T CombineVariants -R $ref -V $dOut/vcf.filter.list -o $dOut/pop.merge.vcf --genotypemergeoption UNSORTED \n";
close SH;
my $job="perl /mnt/ilustre/users/dna/.env//bin/qsub-sge.pl --Resource mem=20G --CPU 1  --maxjob $proc $dShell/step08.vcf-filter2.sh";
print $job;
`$job`;
