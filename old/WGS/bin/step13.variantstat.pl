...
open SH,">$dShell/step13.variant-stat.sh";
open In,$vcflist;
my %pop;
while (<In>) {
	chomp;
...
my $job="perl /mnt/ilustre/users/dna/.env//bin/qsub-sge.pl  --Resource mem=3G --CPU 1 --maxjob $proc  $dShell/step13.variant-stat.sh";
`$job`;
#######################################################################################
