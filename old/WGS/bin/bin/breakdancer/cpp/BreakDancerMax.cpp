...
string version ("BreakDancerMax-1.1.2");
string options ("");
...
int main(int argc, char *argv[])
{
	...
	if(optind == argc){
		fprintf(stderr, "\n");
		fprintf(stderr, "Program: breakdancer_max <analysis.config>\n");
        ...
        fprintf(stderr, "       -r INT          minimum number of read pairs required for a SV [%d]\n", min_read_pair);         
        ...
        fprintf(stderr, "       -t              only detect inter-chromosomal rearrangement, by default off\n");         
        ...
        fprintf(stderr, "       -y INT          output SVs with scores > [%d]\n", score_threshold);
        ...
        while(tails.size() > 0){
            ...
            for(it_tails = tails.begin(); it_tails != tails.end(); it_tails ++){
                ...
                for(vector<int>::iterator ii_s1s = s1s.begin(); ii_s1s != s1s.end(); ii_s1s++){
                    ...
                    vector<map<string, map<string,int> > > type_orient_counts;
                    ...
                    for(vector<int>::iterator ii_snodes = snodes.begin(); ii_snodes != snodes.end(); ii_snodes++){
		        int node = *ii_snodes;
			map<string,map<string,int> > orient_count;
                        vector<vector<string> > nonsupportives;
                        for(vector<vector<string> >::iterator ii_regs = regs[node].begin(); ii_regs != regs[node].end(); ii_regs++){
                            vector<string> y = *ii_regs;

                            if(read_pair.find(y[0]) == read_pair.end()){  //paired
                                // initialize orient_count

                                if(orient_count.find(y[5]) != orient_count.end() && orient_count[y[5]].find(y[3]) != orient_count[y[5]].end())
                                    orient_count[y[5]][y[3]]++;
                                else
                                    orient_count[y[5]][y[3]]=1;
                            }
                            else{
                                nonsupportives.push_back(y);
                            }
                        ...
                        for(map<string,int>::iterator ii_type = type.begin(); ii_type != type.end(); ii_type ++){
                          ...
                          if(type[flag] >= min_read_pair){
                            ...
                            for(vector<int>::iterator ii_snodes = snodes.begin(); ii_snodes != snodes.end(); ii_snodes ++){
                                ...
                                map<string, map<string,int> > ori_readcount = type_orient_counts.front();
                                ...
                                if(sv_chr1 != -1 && sv_chr2 != -1){
                                    ...
                                    if(ori_readcount.find(flag)!=ori_readcount.end() && ori_readcount[flag].find("+") != ori_readcount[flag].end())
                                        //sprintf(sv_ori2_tmp1, "%s", ori_readcount["+"]);
                                        sv_ori2_tmp1 = itos(ori_readcount[flag]["+"]);
                                    if(ori_readcount.find(flag)!=ori_readcount.end() && ori_readcount[flag].find("-") != ori_readcount[flag].end())
                                        //sprintf(sv_ori2_tmp2, "%s", ori_readcount["-"]);
                                        sv_ori2_tmp2 = itos(ori_readcount[flag]["-"]);
