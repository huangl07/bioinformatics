...
open In,$ulist;
open SH,">$dShell/step02.cstacks.sh";
my $ustack;
my $n=0;
while (<In>) {
	chomp;
...
close Out;
my $job="perl /mnt/ilustre/users/dna/.env//bin//qsub-sge.pl --Queue dna --Resource mem=20G --CPU 16 --Nodes 1 $dShell/step02.cstacks.sh";
print "$job\n";
#`$job`;
print "$job\tdone!\n";
