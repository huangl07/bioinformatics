...
if($step==4){
	print Log "########################################\n";
	print Log "remove adapter !"; my $time=time();
	print Log "########################################\n";
	$fqlist=ABSOLUTE_DIR("$dOut/01.RawData/fastq.list");
	my $job="perl $Bin/bin/step04.adapter-cut.pl -fqlist $fqlist -out $dOut/04.AdapterCut/ -dsh $dOut/work_sh";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
	print Log " remove adapter  Done and elapsed time : ",time()-$time,"s\n";
	print Log "########################################\n";
	$step++ ;
}
if($step==5){
	...
	print Log "########################################\n";
	$fqlist=ABSOLUTE_DIR("$dOut/04.AdapterCut/fastq.list");
	my $job="perl $Bin/bin/step05.clean-generic.pl -fqlist $fqlist -out $dOut/05.CleanData/ -dsh $dOut/work_sh";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if($step==6){
	...
	print Log "########################################\n";
	$fqlist=ABSOLUTE_DIR("$dOut/05.CleanData/fastq.list");
	my $job="perl $Bin/bin/step06.clean-qc.pl -fqlist $fqlist -out $dOut/06.CleanDataQC/ -dsh $dOut/work_sh";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if($step==7){
	...
	print Log "########################################\n";
	$fqlist=ABSOLUTE_DIR("$dOut/05.CleanData/fastq.list");
	my $job="perl $Bin/bin/step07.clean-dup.pl -fqlist $fqlist -out $dOut/07.CleanDataDup/ -dsh $dOut/work_sh";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if($step==8){
	...
	print Log "########################################\n";
	$fqlist=ABSOLUTE_DIR("$dOut/05.CleanData/fastq.list");
	my $job="perl $Bin/bin/Pollution/Pollution.pl -fqlist $fqlist -o $dOut/08.Pollution/ -k pollution";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if ($step == 9) {
	...
	my $cleanlist=ABSOLUTE_DIR("$dOut/06.CleanDataQC/stat.list");
	my $cleandup=ABSOLUTE_DIR("$dOut/07.CleanDataDup/dup.list");
	my $adalist=ABSOLUTE_DIR("$dOut/04.AdapterCut/ada.list");
	my $pollution=ABSOLUTE_DIR("$dOut/08.Pollution/pollution.summary.xls");
	my $job="perl $Bin/bin/step08.merge_report.pl -rawlist $rawlist -rawdup $rawdup -cleanlist $cleanlist -cleandup $cleandup -adaplog $adalist -pollution $pollution -out $dOut/09.QCreport -Key $runID && ";
	$job.="perl $Bin/bin/step09.merge_result.pl -i $dOut/09.QCreport -o $dOut/09.QCreport/project\n";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
