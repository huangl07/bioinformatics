...
while (<In>) {
	...
	print SH "/mnt/ilustre/users/dna/.env/stacks-2.1/bin/ustacks -f $fq -o $outdir/ -i $n -M 6 -m 2 -p 8 --deleverage && ";
	print SH "perl $Bin/bin/tag-stat.pl -i $outdir/$sample -o $outdir/$sample.tags.stat -k $sample && ";
	print SH "touch $outdir/$sample.check \n";
	print Out "$sample\t$outdir/$sample\n";
	$n++;
}
close In;
close Out;
my $job="perl /mnt/ilustre/users/dna/.env//bin//qsub-sge.pl --Queue dna --Resource mem=256G --CPU 8  $dsh/step03.ustacks.sh";
print "$job\n";
`$job`;
print "$job\tdone!\n";
