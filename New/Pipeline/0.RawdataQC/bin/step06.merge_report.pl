...
foreach my $sample (sort keys %stat) {
	my $samples=$sample;
	$samples=~s/\:/\t/g;
	print Out join("\t",$samples,$stat{$sample}{rawread}/2,$stat{$sample}{cleanread}/2,$stat{$sample}{rawbase},$stat{$sample}{cleanbase},sprintf("%.2f",100*$stat{$sample}{rawq30}),sprintf("%.2f",100*$stat{$sample}{cleanq30}),sprintf("%.2f",100*$stat{$sample}{rawGC}),sprintf("%.2f",100*$stat{$sample}{cleanGC}),$stat{$sample}{rdup},$stat{$sample}{cdup},sprintf("%.2f",100*$stat{$sample}{ada}),$stat{$sample}{pollutions},$stat{$sample}{pollutionr},$stat{$sample}{pollutionn}),"\n";
}
close Out;
