...
foreach my $info (sort keys %libID) {
	if ($type{$info} eq "RAD") {
		print SH " /mnt/ilustre/users/dna/.env//bin/axe-demux -m 0 -z 6 -b $libID{$info} -t $dOut/$info.split.stat -f $fq1{$info} -r $fq2{$info} -F $dOut/$info -R $dOut/$info \n";
	}else{
		print SH " /mnt/ilustre/users/dna/.env//bin/axe-demux -m 0 -z 6 -c -2 -b $libID{$info} -t $dOut/$info.split.stat -f $fq1{$info} -r $fq2{$info} -F $dOut/$info -R $dOut/$info \n";
	}
}
close In;
close SH;
my $job="perl /mnt/ilustre/users/dna/.env//bin/qsub-sge.pl  $dShell/step01.rawdata-prepair.sh --CPU 1 --Resource mem=24G";
`$job`;
#######################################################################################
