...
foreach my $info (sort keys %libID) {
        ...
        if ($type{$info} eq "RAD") {
                print SH " /mnt/ilustre/users/dna/.env//bin/axe-demux -m 0 -z 6 -b $libID{$info} -t $dOut/$info.split.stat -f $fqfor{$info} -r $fqrev{$info} -F $dOut/$info -R $dOut/$info \n";
        }else{
                print SH " /mnt/ilustre/users/dna/.env//bin/axe-demux -m 0 -z 6 -c -2 -b $libID{$info} -t $dOut/$info.split.stat -f $fqfor{$info} -r $fqrev{$info} -F $dOut/$info -R $dOut/$info \n";
        }
}
close In;
close SH;
