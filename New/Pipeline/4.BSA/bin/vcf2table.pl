...
while (<In>) {
	...
	if (/^#/) {
		...
		print Out join("\t","CHROM\tPOS\tREF\tALT",@out),"\n";
	}else{
		...
		push @out,join("\t",$samples{$P1}{AD},$samples{$P1}{DP},$samples{$P1}{GQ},$samples{$P1}{PL}) if (defined $P2);
		push @out,join("\t",$samples{$P1}{AD},$samples{$P1}{DP},$samples{$P1}{GQ},$samples{$P1}{PL}) if (defined $B1);
		push @out,join("\t",$samples{$P1}{AD},$samples{$P1}{DP},$samples{$P1}{GQ},$samples{$P1}{PL}) if (defined $B2);
		#print Out join("\t",$baseinfo,$vtype,@out,join(";",@ann)),"\n";
		print Out join("\t",$baseinfo,@out,"\n");
	}
