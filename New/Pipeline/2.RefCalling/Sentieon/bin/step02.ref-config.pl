...
mkdir "$out/ref/" if (!-d "$out/ref/");
open Config,">$out/snpEff.config";
print Config "data.dir =$out/\nref.genome : ref\n";
close Config;
`ln -s $ref $out/ref/sequences.fa`;
`ln -s $ref $out/ref.fa`;
open SH,">$dsh/02.ref-config.sh";
print SH "perl $Bin/bin/gffcheck.pl -i $gff -o $out/ref/genes.gff && ";
...
print SH "cd $out && samtools faidx $out/ref.fa && perl $Bin/bin/bed.pl -i $out/ref.fa.fai -o $out/ref.fa.bed\n";
close SH;
