...
if ($step == 7) {
	...
	my $job;
	if($type=~"GATK"){
		$job="perl $Bin/bin/step07.haplotyper.pl -bam $bamlist -ref $ref -dict $dict -out $outdir/07.haplotype -dsh $outdir/work_sh -proc 20";
	}elsif($type=~"samtools"){
		$job="perl $Bin/bin/step07.variant-call-samtools.pl -bam $bamlist -ref $ref -dict $dict -out $outdir/07.haplotype -dsh $outdir/work_sh";
	}elsif($type=~"freebayes"){
		$job="perl $Bin/bin/step07.variant-call-freebayes.pl -bam $bamlist -ref $ref -dict $dict -out $outdir/07.haplotype -dsh $outdir/work_sh";
	}
	print LOG "$job\n";
	`$job`;
	print LOG "$job\tdone!\n";
...
Usage:
	...
	-type	<str>	callsnp's type|GATK or samtools or freebayes	
